...
insert into 과목 values (33,'자바','김상욱','자5-302');
select * from 과목;
...
select 학번, 이름 ,학과명 from 학생, 학과 where 학생.학과번호 = 학과.학과번호;
select 학번, 이름, 학과명 from 학생 s join 학과 w on s.학과번호 = w.학과번호;
select 학번, 이름 학과명
from 학생 s, 학과 d
...
where exists (select *
				from 학과
				where 학과명 = '물리학과' and 학생.학과번호 = 학과.학과번호);
                
select 이름
from 학생
where exists (select *
				from 수강
				where 학생.학번 = 수강.학번 and 과목번호 = 10);
                
select 이름
from 수강, 학생
where 수강.학번 = 학생.학번 and 과목번호=10;
